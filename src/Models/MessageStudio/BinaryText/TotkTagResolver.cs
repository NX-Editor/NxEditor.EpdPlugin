...
public class TotkTagResolver : ITagResolver
{
    public string? GetName(ushort group, ushort type)
    {
        return (group, type) switch {
            ...
            (3, 0) => ResetAnimationTag.NAME,
            (4, 0) => AnimationTag.NAME,
            _ => null
        };
    }
    ...
    public (ushort, ushort)? GetGroupAndType(ReadOnlySpan<char> name)
    {
        return name switch {
            ...
            ResetAnimationTag.NAME => (3, 0),
            AnimationTag.NAME => (4, 0),
            _ => null
        };
    }
    ...
    public bool WriteBinaryUtf16(ref RevrsWriter writer, ushort group, ushort type, in TagParams @params)
    {
        return (group, type) switch {
            ...
            (3, 0) => ResetAnimationTag.WriteUtf16(writer, @params),
            (4, 0) => AnimationTag.WriteUtf16(writer, @params),
            _ => false
        };
    }
    ...
    public bool WriteText(StringBuilder sb, ushort group, ushort type, Span<byte> data)
    {
        return (group, type) switch {
            ...
            (3, 0) => ResetAnimationTag.WriteText(sb, data),
            (4, 0) => AnimationTag.WriteText(sb, data),
            _ => false
        };
    }
}
